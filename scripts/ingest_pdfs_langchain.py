...
class FIALangChainIngestionPipeline:
    ...
    def __init__(self, data_dir: str, output_dir: str, chunk_size: int = 1000, overlap: int = 200):
        """
        ...
        try:
            ...
            output_data = {
                ...
                'processing_stats': {
                    ...
                    'average_chunk_size': sum(chunk['char_count'] for chunk in processed_chunks) / len(processed_chunks),
                    'chunk_size_limit': self.text_splitter._chunk_size,
                    'overlap_size': self.text_splitter._chunk_overlap
                }
            }
            
            logger.info(f"Successfully processed {relative_path}: {len(processed_chunks)} chunks")
            return output_data
    ...
    def run_ingestion(self) -> Dict[str, Any]:
        ...
        for pdf_path in tqdm(pdf_files, desc="Processing PDFs"):
            ...
            if processed_data:
                # Save to JSON
                output_filename = f"{processed_data['document_info']['document_id']}.json"
        ...
        logger.info(f"Ingestion completed: {successful_files}/{len(pdf_files)} files processed successfully")
