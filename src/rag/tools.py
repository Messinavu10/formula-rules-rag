...
class RegulationSearchInput(BaseModel):
    ...
    regulation_type: Optional[str] = Field(None, description="Filter by regulation type (sporting, technical, financial, operational)")


class RegulationComparisonInput(BaseModel):
    """Input for regulation comparison tool."""
    article_number: str = Field(description="The article number to compare (e.g., '5', '12.3')")
    year1: str = Field(description="First year to compare (e.g., '2024')")
    year2: str = Field(description="Second year to compare (e.g., '2025')")
    regulation_type: Optional[str] = Field(None, description="Type of regulation to compare")


class PenaltyLookupInput(BaseModel):
    """Input for penalty lookup tool."""
    violation_type: str = Field(description="Type of violation (e.g., 'track limits', 'MGU-K', 'fuel flow')")
...
class RegulationSearchTool(BaseTool):
    ...
    description: str = "Search FIA regulations for specific information. Use this to find articles, sections, or specific rules."
    ...
    def _run(self, query: str, year_filter: Optional[str] = None, regulation_type: Optional[str] = None) -> str:
...
class RegulationComparisonTool(BaseTool):
    ...
    description: str = "Compare specific articles or sections between different years of FIA regulations."
    ...
    def _run(self, article_number: str, year1: str, year2: str, regulation_type: Optional[str] = None) -> str:
...
class PenaltyLookupTool(BaseTool):
    ...
    description: str = "Look up penalties and sanctions for specific violations in FIA regulations."
    ...
    def _run(self, violation_type: str, year: Optional[str] = None) -> str:
        ...
        try:
            ...
            result = self.rag_pipeline.query(
                question=query,
                year_filter=year,
                regulation_type_filter="sporting",
                k=3
            )
...
class RegulationSummaryTool(BaseTool):
    ...
    description: str = "Summarize and analyze multiple regulations on a specific topic across different years or types."
    args_schema: type = RegulationSearchInput
    rag_pipeline: FIARAGPipeline
...
class GeneralRAGTool(BaseTool):
    ...
    description: str = "General search tool for any FIA regulation questions that don't fit specific tools."
    args_schema: type = RegulationSearchInput
    rag_pipeline: FIARAGPipeline
