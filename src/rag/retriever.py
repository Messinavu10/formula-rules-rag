...
class FIAAdvancedRetriever:
    ...
    def __init__(
        ...
        self.llm = ChatOpenAI(
            openai_api_key=openai_api_key, model="gpt-4.0-mini", temperature=0
        )
    ...
    def retrieve_with_metadata(
        self,  # return raw document chunks with metadata for full context and filtering capabilities
        query: str,
        k: int = 5,
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(formatted_results)} documents for query: '{query}'"
            )
    ...
    def retrieve_compressed(
        ...
        Dict[str, Any]
    ]:  # uses LL to extract only most relevant parts, reduces noise
        """
    ...
    ) -> str:  # create a formatted citation from metadata
    ...
    def _format_source_info(
        self, metadata: Dict[str, Any]
    ) -> str:  # format source information for display
