...
class FIAAgentEvaluator:
    ...
    def _get_agent_responses(self, dataset: Dataset, tool_name: str) -> List[Dict[str, Any]]:
        """
    ...
    def _generate_comprehensive_report(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if total_metrics > 0:
            ...
            report["summary"]["total_tools_evaluated"] = len([r for r in results.values() if "error" not in r])
            report["summary"]["total_metrics"] = total_metrics
        
        # Generate recommendations
        report["recommendations"] = self._generate_recommendations(report["tool_performance"])
    ...
    def _generate_recommendations(self, tool_performance: Dict[str, Any]) -> List[str]:
        ...
        for tool_name, metrics in tool_performance.items():
            ...
            if len(high_scores) == len(metrics):
                recommendations.append(f"{tool_name} is performing excellently across all metrics")
